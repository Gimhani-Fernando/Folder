...
function setc($coname,$coval){
    setcookie($coname,"",-1000);
}
?>
<!DOCTYPE html>
<html lang="en">
...
<?php
    ...
    if (isset($_POST['submit'])){
        ...
        if ($result_count>0){
            ...
            if($list1['password']==md5($pw)){
                $sql="DELETE FROM `user` WHERE `user`.`username` = '{$uname}';";
                $statement=$connect->prepare($sql);
        ...
        echo 
    '<form style="border:1px black solid;width:400px;" action="deleteuser.php" method="post">
        <table style="text-align:right">
            <tr style="text-align:center">
                <th colspan="2"><h1>Delete Account</h1></th>
            </tr>
            <tr>
                <th>
                    User Name :
